...
import { isPriority, type Priority, type Task } from '../models/index.js';
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  if (('priority' in req.body)) {
    const incomingPriority: string = String(req.body.priority).toLowerCase();
    if (!isPriority(incomingPriority)) {
      res.status(422).json({
        status: 'Unprocessable Entity',
        message: 'Failed to create a new task',
    ...
    priority = incomingPriority as Priority;
  }

  try {
    const createdTask: Task = await tasksService.create(content, priority);
    res.status(201).json({
      status: 'Created',
